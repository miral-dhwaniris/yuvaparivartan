...
//                    'delete' => ['post'],
    ...
    public function actionDatalist() {
        ...
        $responseArray['table_name'] = "district";
        $milliseconds = round(microtime(true) * 1000);
        $responseArray['timestamp'] = $milliseconds;
        $modelBlank = new District();
        $responseArray["labels"] = $modelBlank->attributeLabels();

        if (isset($_GET["timestamp"])) {
            $model = District::find()->asArray()->where(['>', 'timestamp', $_GET["timestamp"]])->all();
        } else {
            $model = District::find()->asArray()->all();
        }
